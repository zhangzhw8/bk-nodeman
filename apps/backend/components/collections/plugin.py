...
class GseOperateProcService(PluginBaseService):
    ...
    def get_resource_policy(bk_host_ids: Set[int], plugin_name: str) -> Dict[int, Dict]:
        ...
        # 查询主机对应的服务模板ID列表
        try:
            host_service_templates = CmdbHandler.find_service_template_by_host(list(bk_host_ids))
        except ComponentCallError as error:
            # 接口不存在时，使用默认配置
            logger.exception(
                "call find_service_template_by_host error: {error}," " use default resource policy".format(error=error)
            )
